...
<script type="text/javascript">
 $(function() {
		...
		$('#table').bootstrapTable({
			url : '/section/findSectionByProfessorId',
			queryParamsType:'', 
...
// 				  };
		...
		function childTable(index, row, $detail){
			 	var parentid = row.sectionNo;
			    var group_com = $detail.html('<table></table>').find('table');
			    $(group_com).attr("id","CourseNo");
			    $(group_com).bootstrapTable('destroy');
			    $(group_com).bootstrapTable({
			    	url: '/section/findStudentBySectionNo',
			    	method: 'get',
			    	queryParams: { sectionNo: parentid },
			    	ajaxOptions: { sectionNo: parentid },
			    	sidePagination : 'server',
			    	clickToSelect: true,
			    	cache :　false,
			    	uniqueId: "",
